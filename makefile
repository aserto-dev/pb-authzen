...
# BUF_REPO           := "buf.build/aserto-dev/check2decision"
# BUF_LATEST         := $(shell BUF_BETA_SUPPRESS_WARNINGS=1 ${EXT_BIN_DIR}/buf beta registry label list buf.build/aserto-dev/check2decision --format json --reverse | jq -r '.results[0].name')
# BUF_DEV_IMAGE      := "${BIN_DIR}/check2decision.bin"
...
buf-breaking:
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@${EXT_BIN_DIR}/buf breaking proto --against "https://github.com/aserto-dev/check2decision#branch=main"

.PHONY: buf-build
buf-build: ${BIN_DIR}
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@${EXT_BIN_DIR}/buf build proto --output ${BIN_DIR}/check2decision.bin
...
buf-generate:
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@${EXT_BIN_DIR}/buf generate

.PHONY: buf-generate-dev
buf-generate-dev:
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@${EXT_BIN_DIR}/buf generate "${BIN_DIR}/check2decision.bin"
